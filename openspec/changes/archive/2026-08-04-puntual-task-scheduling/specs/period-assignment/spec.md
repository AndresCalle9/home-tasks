...
(`is_daily = false`), whether fixed or variable, SHALL receive 3 distinct
days of the week as part of this same run; tasks sharing the same non-null
`day_group` SHALL always receive the identical set of 3 days as each other.
After the lottery, the system SHALL rebalance by repeatedly transferring one
...
#### Scenario: Once-per-period tasks get 3 days a week
- **WHEN** the assignment runs and a task has `is_daily = false`
- **THEN** the system SHALL assign that task 3 distinct days of the week
...
- **THEN** the system SHALL assign the identical set of 3 days to every
  task in that group
...
- **THEN** the system SHALL NOT require its 3 days to match any other
  task's days
