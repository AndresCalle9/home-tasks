...
period and in previously assigned periods), such that a member holding
fewer tasks has a proportionally higher chance of being picked. A task
with a configured minimum age SHALL only be eligible for members at or
above that age; if no member meets it, the system SHALL assign the task
to the oldest household member instead of leaving it unassigned. Age
SHALL NOT otherwise influence the odds of being picked. Fixed tasks SHALL
be assigned directly to their configured member and SHALL NOT enter the
...
the system SHALL ensure every member has at least one variable-task
assignment by transferring one eligible task from whichever member
currently holds the most tasks, for any member left with none. The
algorithm SHALL be deterministic for a given stored seed.
...
- **THEN** each eligible member's chance of being picked SHALL depend only
  on how many tasks they already hold, not on their age
...
- **THEN** the system SHALL transfer one such task from the member
  currently holding the most tasks to the member with none
